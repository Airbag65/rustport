...
impl Command for GetCommand {
    fn execute(&self) -> Result<(), anyhow::Error> {
        ...
        let password: String = nm.get(self.value.clone()).unwrap_or("".to_owned());
        cprintln!("<green>Password for '{}' has been found</>", &self.value);
        cprintln!(
            "<red>WARNING! Be careful when revealing the password. The password will be printed to the terminal window</>"
        );
